...
echo " "
tput setaf 4;
echo "Installing Bolt-Rengine and it's dependencies"

echo " "
if [ "$EUID" -ne 0 ]
  then
  tput setaf 1; echo "Error installing Bolt-Regine, Please run this script as root!"
...
tput setaf 4;
echo "#########################################################################"
echo "Installing Bolt-Rengine"
echo "#########################################################################"
make certs && make build && make up && tput setaf 2 && echo "Bolt-Rengine is installed!!!" && failed=0 || failed=1
...
if [ "${failed}" -eq 0 ]; then
  ...
  tput setaf 2 && printf "\n%s\n" "Thank you for installing Bolt-Rengine, happy recon!!"
else
  tput setaf 1 && printf "\n%s\n" "Bolt-Rengine installation failed!!"
fi
